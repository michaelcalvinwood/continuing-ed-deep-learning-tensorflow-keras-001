...
# define input
input = Input(shape=(max_sequence_size,)) # the shape is a tuple containing 500 values (max_sequence_size) with each value between 0 to 22

# define output
embedding = Embedding(num_amino_acids, embedding_dims)(input) # We reduce the 23 dimensions (e.g. 0-22) down to 10 resulting in shape (None, 500, 10)
x = Flatten()(embedding) 
x = Dense(25, activation='sigmoid')(x)
x = Dense(1)(x)
output = Activation('sigmoid')(x)

# create the model based on input and output
